...
fn test_extract_json_plain() {
    let content =
        r#"{"segments": [{"timestamp": "00:00", "content": "Test"}], "full_text": "Test"}"#;
...
fn test_full_subtitle_extraction_flow() {
    ...
    let api_key = std::env::var("GEMINI_API_KEY").expect("请设置 GEMINI_API_KEY 环境变量");
    let video_path = std::env::var("TEST_VIDEO_PATH").expect("请设置 TEST_VIDEO_PATH 环境变量");
