...
export function NewArticleDialog({ isOpen, onClose, onSave }: NewArticleDialogProps) {
  ...
  return (
    <Dialog isOpen={isOpen} onClose={handleClose} title={t("newArticle.title")}>
      <DialogContent>
        {error && (
      ...
      <DialogFooter>
        <Button variant="secondary" onClick={handleClose} disabled={isSaving || isFetching}>
          {t("newArticle.cancel")}
        </Button>
...
export function NewArticleButton({ onSave }: NewArticleButtonProps) {
  const { t } = useTranslation();
